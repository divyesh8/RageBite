...
export default function RegisterPage() {
  ...
  const onUsernameInput = (val) => {
    ...
    if (val.length >= 3 && USER_RE.test(val)) {
      ...
      usernameTimer.current = setTimeout(async () => {
        try {
          const { data } = await axios.post(`${API}/auth/check-username`, { username: val });
  ...
  if (step === 'otp') {
    return (
      <AuthLayout tag="EMAIL VERIFICATION" title="CHECK YOUR EMAIL" subtitle={`OTP sent to ${userEmail}`}>
        <div className="card space-y-6">
          <div className="text-center">
            <div className="text-4xl inline-block">📧</div>
            <p className="font-mono text-[11px] text-rb-muted mt-2">Expires in 10 minutes · One time use</p>
          </div>
          <div className="flex gap-2 justify-center" onPaste={onOtpPaste}>
            {otp.map((d, i) => (
              <input key={i} ref={otpRefs[i]}
                ...
                style={{width:48,height:56,textAlign:'center',fontFamily:'monospace',
                  fontSize:20,fontWeight:'bold',background:'#0d0d0d',
                  border:`1px solid ${otpError?'#FF3C1A':d?'#00E676':'#252525'}`,
                  borderRadius:12,outline:'none',color:'#F2EEE8'}}
              />
            ))}
          </div>
          {otpError && <p style={{color:'#FF3C1A',fontFamily:'monospace',fontSize:11,textAlign:'center'}}>⚠ {otpError}</p>}
    ...
    <AuthLayout tag="JOIN THE ARENA" title="SIGN UP" subtitle="Create your account and start earning Aura.">
      <div className="card">
        <form onSubmit={handleSubmit} noValidate className="space-y-4">
          <InputField label="USERNAME" name="username" value={fields.username}
          ...
          />
          <InputField label="EMAIL" name="email" type="email" value={fields.email}
          ...
          />
          <div>
            <InputField label="PASSWORD" name="password" type="password" value={fields.password}
          ...
          <InputField label="CONFIRM PASSWORD" name="confirmPassword" type="password" value={fields.confirmPassword}
